...
đó ghi `ID POST`, `Post Link` và trạng thái ngược lại Sheet.
...
| Cột | Loại | Công dụng |
|---|---|---|
| `ID PAGE` | bắt buộc | Page cần đăng bài |
...
| `FB Video ID` | tự ghi | Giúp chạy lại mà không upload trùng |
| `ID POST` | tự ghi | ID dùng cho repo tạo Campaign |
| `Post Link` | tự ghi | Link gửi nhân viên |
| `Post Status` | tự ghi | Tiến độ hoặc lỗi |

Nếu các cột đầu ra chưa tồn tại, chương trình tự thêm chúng vào cuối hàng 1.
Dòng đã có `ID POST` hoặc `Post Link` sẽ được bỏ qua.
...
Ngay sau khi upload xong, chương trình ghi `FB Video ID`. Nếu Meta vẫn đang xử
